...
class Tooltip extends React.Component<TooltipProps, TooltipState> {
  ...
  wrapWithPress = (toggleOnPress: TooltipProps['toggleOnPress'], toggleAction: TooltipProps['toggleAction'], children: React.ReactNode) => {
    if (toggleOnPress) {
      return (
        <TouchableOpacity {...{ [toggleAction]: this.toggleTooltip }} delayLongPress={250} activeOpacity={1}>
          {children}
        </TouchableOpacity>
      );
    }
  ...
  renderTouchableHighlightedButton = () => {
    ...
    return (
      <TouchableOpacity testID="tooltipTouchableHighlightedButton" onPress={() => this.toggleTooltip()} style={TooltipHighlightedButtonStyle}>
        {this.props.children}
      </TouchableOpacity>
    );
  };
